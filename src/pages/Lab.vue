...
export default {
  ...
  data() {
    return {
      projects: [], // Variabile per memorizzare i progetti
      currentPage: 1,
      totalPages: 0, 
    };
  },
  methods: {
    getApi(page = 1) {
      axios
        .get(`http://localhost:8000/api/projects?page=${page}`) 
        .then(result => {
    ...
    changePage(page) {
      this.getApi(page); // Cambia pagina e ottieni i nuovi dati
    }
  },
  mounted() {
    this.getApi(); // Chiamata all'API quando il componente è montato
  }
};
</script>
